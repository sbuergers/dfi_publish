...
[pc_mat, numcor_mat, outof_mat] = deal(nan(size(pffit,2), 3, 8));
for isubj = 1:length(pffit)
    for icond = 1:length(pffit{isubj})
        ...
        pc_mat(isubj, icond, :)     = pffit{isubj}{icond}.perCor;
        numcor_mat(isubj, icond, :) = pffit{isubj}{icond}.NumCorr;
        outof_mat(isubj, icond, :)  = pffit{isubj}{icond}.OutOfNum;
    end % condition
end % subject
