function dataPF = fitPSFmodel_guessFixed(data,betaBool)
...
lapseLimits = [0 1];                        %lapse rate (since we use 'gammaEQlambda', guessLimits will not be used)
...
BootstrapN = 0;                          %Set to 5000 for accurate results             
...
for i=1:NumCond
    paramsFitted_independent(i,:) = PAL_PFML_Fit(StimLevels(i,:),NumPos(i,:),OutOfNum(i,:),searchGrid,paramsFree,PF,...
                                        'gammaEQlambda',0,'lapseLimits',lapseLimits,'searchOptions',searchOptions);
end
...
paramsIDmatrix = [1:3; 4:6; 10 10 10]'; % don't look for guess-rate
InitialParamsVector = [paramsFitted_independent(:,1)' paramsFitted_independent(:,2)' ...
                       paramsFitted_independent(:,3)' mean(paramsFitted_independent(:,4))];
...
while (nFitTries < 10) && (exitflag == 0)
    [paramsFittedVector, negLL, exitflag, output] = PAL_minimize(@DM_PFML_negLLMultiple_Beta_guessFixed, InitialParamsVector2, searchOptions, ...
                                                                 paramsIDmatrix, StimLevels, NumPos, OutOfNum, PF, lapseLimits, betaBool);  
    nFitTries = nFitTries+1;
    InitialParamsVector2 = paramsFittedVector+1e-4;
