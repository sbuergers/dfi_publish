function handles = distributionPlot_DM(varargin)
%This is an adaptation on the original distributionPlot
%Use this function if you want to plot distributions side by side
%There are only minimal changes comared to the original script and all 
%original functionality should still be available.
%
%You can now set 'histOri' and 'widthDiv' to be a cell-array with the same 
%length as nData (similar to what was already possible for 'color').
%Thereby you can thus set the left or right for every dataset in 'data'.
%Note that you should also adjust the 'xvalues' vector accordingly.
%
% EXAMPLE USE:
%
% data = {randn(100,1), randn(500,1), randn(100,1)+2, randn(500,1)+2, (randn(100,1)+4)*4, (randn(500,1)+4)*4, (randn(100,1)-2)./3, (randn(500,1)-2)./3}; 
%     
% histOri = {'left','right','left','right','left','right','left','right'};
% widthDiv = {[2 1],[2 2],[2 1],[2 2],[2 1],[2 2],[2 1],[2 2]};
% colors = {'b','r','b','r','b','r','b','r'};
% x_values = [-10.5 -10.5 -3.5 -3.5 3.5 3.5 10.5 10.5];
%
% distributionPlot_DM(data,'globalNorm',3,'histOri', histOri, 'showMM', 0,'widthDiv',widthDiv, 'color', colors, 'xValues',x_values,'distWidth', 6);
%
% *************************************************************************
%
...
%
% EXAMPLES
%         addpath('E:\3. Matlab\3. Shared Scripts\plotSpread');  
%           
...
for iData = goodData'
    ...
    % DM %
    %%%%%%
    if iscell(opt.widthDiv)
        widthDiv = opt.widthDiv{iData};
    else
        widthDiv = opt.widthDiv;
    end
    
    if iscell(opt.histOri)
        histOri = opt.histOri{iData};
    else
        histOri = opt.histOri;
    end
    
    % get base x-array
    % widthDiv is a 1-by-2 array with [#ofDivs, whichDiv]
    % The full width (distWidth) is split into #ofDivs; whichDiv says which "stripe" is active
    xWidth = opt.distWidth/widthDiv(1);
    xMin = -opt.distWidth/2;
    xLow = xMin + xWidth * (widthDiv(2)-1);
    xBase = [-xWidth;xWidth;xWidth;-xWidth]/2;
    xOffset(iData) = xLow + xWidth/2;
    ...
    % x is iData +/- almost 0.5, multiplied with the height of the histogram
    if opt.variableWidth
...
minX = min(opt.xValues)-opt.distWidth;
maxX = max(opt.xValues)+opt.distWidth;
