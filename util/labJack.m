...
%> Written by IanA
%>
%> 2014-06-17 modified by A Mihalik, CNCR, Birmingham, UK 
%> new functions: getDIO, getDIOValue, getDIODirection
%> modified functions: rawRead
%>
%> The new functions make the class useful also for receiving triggers e.g. 
%> from MRI scanner:
%>
...
classdef labJack < handle
	...
	%=======================================================================
        ...
        function prepareStrobe(obj,value,mask,sendNow)
            ...
            elseif ispc
                % only EIO0-7 can be used at the moment
                if value > 255
                    obj.salutation('prepareStrobe', 'Use of EIO0-3 not implemented under Windows', true)
                else
                    error = calllib('labjackud', 'AddRequest', obj.handle, 45, 8, value, 8, 0); % IOType for LJ_ioPUT_DIGITAL_PORT
                    if error
                        calllib('labjackud', 'ErrorToString', error, '')
                    end
                end
		...
		function strobeWord(obj)
            ...
            elseif ispc
                ...
                error = calllib('labjackud','GetResult', obj.handle, 45, 8, 0); % IOType for LJ_ioPUT_DIGITAL_PORT
                if error
                    calllib('labjackud', 'ErrorToString', error, '')
                end
            end
