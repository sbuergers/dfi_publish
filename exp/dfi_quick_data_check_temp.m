function dfi_quick_data_check_temp( data, paradigm, distr, save_figures )
% dfi_quick_data_check( data, paradigm, distr, save_figures )
...
elseif strcmp(paradigm, 'yesno')
    ...
    header = '2F1S trials (fusion)';
    pffit{2} = dfi_fit_pf(dall(dall.trlid == 6,:), dist, sg, fparams, 0, header);
    
    header = '1F2S trials (fission)';
    pffit{3} = dfi_fit_pf(dall(dall.trlid == 8,:), dist, sg, fparamsfis, 0, header);
    
    header = '2F2S trials (fission control)';
    pffit{4} = dfi_fit_pf(dall(dall.trlid == 9,:), dist, sg, fparams, 0, header);
    
    % Assume same lapse rate for all conditions
    estimFit = 0;
    numRep   = 100;
    outp = dfi_fit_pf_group(dall(ismember(dall.trlid, [3,6,8,9]),:), dist, sg, fparams, estimFit, numRep);
    ...
    header = '1F & 2F trials';
    dsamaha = dall;
    dsamaha.trlid(dsamaha.trlid == 2) = 3;
    sgsamaha = sg;
    sgsamaha.gamma = 0.5;
    sgfparams = [1 1 0 1];
    pffit_sam{1} = dfi_fit_pf(dsamaha(dsamaha.trlid == 3,:), dist, sgsamaha, sgfparams, 0, header);
    
    % Fusion
    header = '1F1S & 2F1S trials (fusion)';
    dsamaha.trlid(dsamaha.trlid == 5) = 6;
    sgsamaha = sg;
    sgsamaha.gamma = 0.5;
    sgfparams = [1 1 0 1];
    pffit_sam{2} = dfi_fit_pf(dsamaha(dsamaha.trlid == 6,:), dist, sgsamaha, sgfparams, 0, header);
    
    % Fission
    header = '1F2S & 2F2S trials (fission)';
    dsamaha.trlid(dsamaha.trlid == 8) = 9;
    sgsamaha = sg;
    sgsamaha.gamma = 0.5;
    sgfparams = [1 1 0 1];
    pffit_sam{3} = dfi_fit_pf(dsamaha(dsamaha.trlid == 9,:), dist, sgsamaha, sgfparams, 0, header);
    
    % Assume same lapse rate for all conditions
    estimFit = 0;
    numRep   = 100;
    outpsamaha = dfi_fit_pf_group(dsamaha(ismember(dsamaha.trlid, [3,6,9]),:), dist, sg, fparams, estimFit, numRep);
    ...
    [counts1, binCenters1] = hist(dall.RT(dall.trlid == 3), 10);
    [counts2, binCenters2] = hist(dall.RT(dall.trlid == 6), 10);
    [counts3, binCenters3] = hist(dall.RT(dall.trlid == 8), 10);
    [counts4, binCenters4] = hist(dall.RT(dall.trlid == 9), 10);
    plot(binCenters1, counts1, 'g-'); hold on;
    plot(binCenters2, counts2, 'b-');
    plot(binCenters3, counts3, 'r-'); 
    plot(binCenters4, counts4, 'k-'); xlim([0 2])
    title(sprintf('Participant %g, session %s', dall.partid(1), num2str(unique(dall.sess))));
    xlabel('Response time (s)'); ylabel('Number of responses');
    suptitle(sprintf('Response times'));
    plotspecs; grid on;
    legend('2F','2F1S','1F2S','2F2S','location','northeast')
    ...
    for isoa = 1:Nsoa
        ...
        [counts1, binCenters1] = hist(dall.RT(dall.trlid == 3 & dall.soa == SOAs(isoa)), 10);
        [counts2, binCenters2] = hist(dall.RT(dall.trlid == 6 & dall.soa == SOAs(isoa)), 10);
        [counts3, binCenters3] = hist(dall.RT(dall.trlid == 8 & dall.soa == SOAs(isoa)), 10);
        [counts4, binCenters4] = hist(dall.RT(dall.trlid == 9 & dall.soa == SOAs(isoa)), 10);
        plot(binCenters1, counts1, 'g-'); hold on;
        plot(binCenters2, counts2, 'b-');
        plot(binCenters3, counts3, 'r-');
        plot(binCenters4, counts4, 'k-'); xlim([0 2])
    ...
    if save_figures
        ...
        saveas(pffit{2}.fh, fullfile(fig_dir, sprintf('PF_Fus_%s.emf', distr)))
        saveas(pffit{3}.fh, fullfile(fig_dir, sprintf('PF_Fis_%s.emf', distr)))
        saveas(pffit{4}.fh, fullfile(fig_dir, sprintf('PF_2F2S_%s.emf', distr)))
        saveas(outp.fh,     fullfile(fig_dir, sprintf('PFs_%s.emf', distr)))
        saveas(pffit_sam{1}.fh, fullfile(fig_dir, sprintf('PF_samaha_2F_%s.emf', distr)))
        saveas(pffit_sam{2}.fh, fullfile(fig_dir, sprintf('PF_samaha_Fus_%s.emf', distr)))
        saveas(pffit_sam{3}.fh, fullfile(fig_dir, sprintf('PF_samaha_Fis_%s.emf', distr)))
        saveas(outpsamaha.fh,   fullfile(fig_dir, sprintf('PFs_samaha_%s.emf', distr)))
        saveas(fh_data_quality, fullfile(fig_dir, sprintf('Data_quality.emf')))
        close all
    end
