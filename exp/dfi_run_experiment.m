...
if strcmp(s.paradigm, 'yesno')                                       % 2 - 10  = V1
    ...
    elseif strcmp(s.cond, 'audio')                                   % 7 - 02  = A1A2
        s.stim.trlid = [4 7 4 7 4 7 4 7 4 7 4 7];                    % 8 - 12  = V1A2
...
elseif strcmp(s.paradigm, '2AFC') || strcmp(s.paradigm, '2AFCsoundTest') % \2AFC\
    ...
    elseif strcmp(s.cond, 'audio')                              % 6 - V2A1-V1A1  =  Fus
        s.stim.trlid = [4 7 4 7 4 7 4 7 4 7 4 7];               % 7 - A1A2-A1    =  A
...
s.ntrls      = numel(s.stim.SOA)*numel(s.stim.trlid) + 1; % per block (plus one nil trial)
    s.blocks.N   =  12; % nblocks               % ntrls    soa    conds
...
if strcmp(s.paradigm, '2AFC')
    ...
    s.blocks.N   =  12; % change back to 12!  
...
% auditory only condition for fitting PF 
if strcmp(s.cond, 'audio')
    s.stim.SOA = [10:7.5:40,75]/1000; % yesno
...
if strcmp(s.paradigm, 'YN_threshold')
    ...
    s.ntrls      = 12 * 6 + 1; % 1 nil trials per block
    s.stim.trlid = [2 3 5 6 8 9]; 
end
...
s.stair.nBurnin = 5;
s.stair.nRevers = 5;
s.stair.nTrials = 10; % have 5 reversals within 10 trials,
% lower bound of expected value after convergence is 13 trials, which is a
% little more than 2 blocks for 2 chains. Second bound will be the number
% of blocks (say 4).
...
if strcmp(s.stair.incl, 'yes')
    ud_general = PAL_AMUD_setupUD(...
        ...
        'stepSizeUp', 1/120, ...
        'stepSizeDown', 1/120, ...
        'stopRule', s.stair.nRevers);
    % starting values are estimated from previous YN data. Restrict chains
    % within range +- 5 flips, and of course min(soa) == 1 flip
    for ichains = 1:s.stair.nChains
        if s.stim.SOA_FF/(1/120) - 5 < 1
            xMin = 1*(1/120);
        else
            xMin = s.stim.SOA_FF - 5*(1/120);
        end
        s.ud{ichains}.FF  = PAL_AMUD_setupUD(ud_general, 'startValue', s.stim.SOA_FF, ...
                                                          'xMax', s.stim.SOA_FF + 5*(1/120), ...
                                                          'xMin', xMin);
        if s.stim.SOA_Fus/(1/120) - 5 < 1
            xMin = 1*(1/120);
        else
            xMin = s.stim.SOA_Fus - 5*(1/120);
        end
        s.ud{ichains}.Fus = PAL_AMUD_setupUD(ud_general, 'startValue', s.stim.SOA_Fus, ...
                                                          'xMax', s.stim.SOA_Fus + 5*(1/120), ...
                                                          'xMin', xMin);
        % restricting Fission is difficult so let's not do it due to weird
        % PF shapes that can occur. 
        xMin = 1*(1/120);
