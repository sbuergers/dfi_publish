% Figure and analysis script
% Between subject analyses
% Control: Only allow threshold estimates < 225 ms
%
% Parent script(s): 
%
...
% Assign nan to threshold estimates at 225ms
threshold_2ifc(threshold_2ifc > 0.224999) = nan;
threshold_ynpool(threshold_ynpool > 0.224999) = nan;
...
saveas(fh0,fullfile(save_dir, 'eo_sens_thresh_scatter_freq.svg'))
...
fh02 = figure('color', [1 1 1], 'Position', [0, 0, 427, 350]);
ha = tight_subplot(3, 4, [0.01 0.03], [0.02], [0.02]);
...
for icond = 1:3
    ...
    % For now, exclude non-sensical / clearly bad fits
    bad_fit  = isnan(ip) | repmat(~any(~isnan(peak_mat_occ_source),2), [1, 3]);
    good_fit = ~bad_fit & ~(ip > 0.5);
    ...
    % Correlate iAF with perceptual windows for all tasks (most importantly dfi)
    percwin = ones(size(peak_mat_occ_source))./peak_mat_occ_source;
