...
[threshold_matrix, slope_matrix, lapse_matrix] = deal(nan(size(pffit_yn_pooled,1), 3));
[pc_mat] = nan(size(pffit_yn_pooled,1), 3, 8);
for isubj = 1:length(pffit_yn_pooled)
    for icond = 1:length(pffit_yn_pooled{isubj})
        if ~reject(isubj, icond)
...
pc_GA_yn_pool = squeeze(nanmean(pc_mat,1));
pc_SE_yn_pool = squeeze(nanstd(pc_mat)) ./ sqrt(size(pc_mat,1));
...
[threshold_matrix, slope_matrix, lapse_matrix] = deal(nan(size(pffit_2ifc,1), 3));
[pc_mat, numcor_mat, outof_mat] = deal(nan(size(pffit_2ifc,1), 3, 8));
...
%% Figure 2IFC and YN pooled psychometric functions
