...
for isubj = 1:N
    ...
    for isess = 1:10
        for irun = 1:10
            ...
            if exist(fullfile(temp_dir, eegfile), 'file')
                ...
                % cutout relevant time window (to save mem.)
                cfg          = [];
                cfg.toilim   = [-1.2, 0.7];
    ...
    for isess_combi = ncomb
        ...
        % get PSD per condition and percept        
        s = dfi_eeg_defaults;
        s.chan      = {'O*', 'POz', 'PO4', 'PO8'}; % Electrode with max alpha at group level: POz
        s.psd       = 0;
        s.tfr       = 0;
        s.plv       = 0;
        s.phase     = 0;
