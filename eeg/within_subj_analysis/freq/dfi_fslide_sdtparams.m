...
%
% Children script(s): 
%   dfi_clusterPerm_dprime_no_regress_freq.m
%   dfi_clusterPerm_criterion_no_regress_freq.m
%
% Sibling script(s):
...
fh1 = figure('color', [1 1 1], 'Position', [0, 0, 1000, 800]);
...
for icond = 1:6
    ...
    if icond == 1 || icond == 4, ylabel('Frequency (Hz)'); end
    if icond > 3, xlabel('Time (s)'); end
end


% get GA and standard error of the mean after pooling over perceptual
% outcomes and conditions!
fslide_poolall_GA_uncorr = nanmean(nanmean(fslide_subj_uncorr(:,:,:),1),2);
fslide_poolall_SE_uncorr = nanstd(nanmean(fslide_subj_uncorr(:,:,:),1)) ./ sqrt(20);

% 1.) Plot avg fslide over all conditions
fh1 = figure('color', [1 1 1], 'Position', [0, 0, 300, 220]);
tid = toi > -0.602 & toi < -0.102;
tif = toi(tid);
shadedErrorBar(tif', fslide_poolall_GA_uncorr(:,tid), fslide_poolall_SE_uncorr(:,tid), {'-b','markerfacecolor', 'b'}, 1); hold on
% accentuate actual GA time courses
plot(tif', fslide_poolall_GA_uncorr(:,tid),  'color', col2{icond}, 'linewidth', 1.5); hold on; % correct
xlim([-0.6 -0.1])
ylim([9 10.5])
title('Average frequency')
ylabel('Frequency (Hz)');
xlabel('Time (s)');
addtext(sprintf('mean freq = %f2 Hz', mean(fslide_poolall_GA_uncorr(:,tid))))


% 1.) Plot avg fslide over all conditions for each subject
fh1 = figure('color', [1 1 1], 'Position', [0, 0, 1200, 900]);
for isubj = 1:20
    subplot(4,5,isubj)
    tid = toi > -0.602 & toi < -0.102;
    tif = toi(tid);
    subj_avg = squeeze(nanmean(fslide_subj_uncorr(:,isubj,:),1));
    % accentuate actual GA time courses
    plot(tif', subj_avg(tid),  'color', 'b', 'linewidth', 1.5); hold on; % correct
    xlim([-0.6 -0.1])
    ylim([8 11])
    title(sprintf('Subject %i', isubj))
    ylabel('Frequency (Hz)');
    xlabel('Time (s)');
    addtext(sprintf('mean freq = %f2 Hz', mean(subj_avg(tid))))
end
