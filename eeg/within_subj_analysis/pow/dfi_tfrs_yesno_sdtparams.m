...
save('D:\dfi_experiment_data\eeg_data\experiment\tfr_yesno_table_alpha_only.mat', ...
...
load('D:\dfi_experiment_data\eeg_data\experiment\tfr_yesno_table_alpha_only.mat', ...
...
% 
% 
% % Power histograms separately for sessions, for each participant
% figure('color', 'w', 'position', [50 50 1000 900])
% for isubj = 1:20
%     subplot(5,4,isubj)
%     hist(log10(btable.avgpow_alpha(btable.partid == partvect(isubj) & ...
%         btable.sessid == 1,:)),1000);
%     xlim([-1 3])
%     xlabel('log-power')
%     ylabel('count')
%     title(sprintf('Subj %i', isubj));
% end
% suptitle('Power histograms sessions 1')
% 
% figure('color', 'w', 'position', [50 50 1000 900])
% for isubj = 1:20
%     subplot(5,4,isubj)
%     hist(log10(btable.avgpow_alpha(btable.partid == partvect(isubj) & ...
%         btable.sessid == 2,:)),1000);
%     xlim([-1 3])
%     xlabel('log-power')
%     ylabel('count')
%     title(sprintf('Subj %i', isubj));
% end
% suptitle('Power histograms sessions 2')
...
% Go through each SOA separately and compute SDT parameters
soas = unique(btable.soa);
[dp_cell, c_cell, pow_cell] = deal(cell(length(soas),nt-1));
...
pow_mat_cont = nan([length(soas),nt-1,size(pow_mat)]);

for isoa = 1:length(soas)
    for t = 1:nt-1
        dp_mat_cont(isoa,t,:,:,:) = dp_cell{isoa,t};
        c_mat_cont(isoa,t,:,:,:) = c_cell{isoa,t};
        pow_mat_cont(isoa,t,:,:,:) = pow_cell{isoa,t};
    end
end
...
tvect = time_windows(2:nt)-diff(time_windows);
if size(pow_mat_cont,1) == 8
    pow_mat_cont = squeeze(nanmean(pow_mat_cont(3:6,:,:,:,:),1)); % average over interm. soas
end
